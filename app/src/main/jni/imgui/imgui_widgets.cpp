...
bool ImGui::OptButtonEx1(const char* ico, const ImVec2& size_arg, bool rotation, ImGuiButtonFlags flags)
{
	...
	ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(255/255.0f, 255/255.0f, 255/255.0f, it_nameanim->second));
...
bool ImGui::OptButtonEx2(const char* ico, const ImVec2& size_arg, bool rotation, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.text1 = ImLerp(it_anim->second.text1, (hovered ? 21.f : 30.f), 0.10f * (1.0f - ImGui::GetIO().DeltaTime));
 	...
 	RenderFrame(ImVec2(bb.Min.x + it_anim->second.sizex1, bb.Min.y + it_anim->second.sizey1), ImVec2(bb.Max.x - it_anim->second.sizex1, bb.Max.y - it_anim->second.sizey1), ImColor(220, 223, 228, 255), true, 10.f);
...
bool ImGui::OptButtonEx3(const char* ico, const ImVec2& size_arg, bool rotation, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    it_anim->second.text1 = ImLerp(it_anim->second.text1, (hovered ? 21.f : 30.f), 0.10f * (1.0f - ImGui::GetIO().DeltaTime));
 	...
 	RenderFrame(ImVec2(bb.Min.x + it_anim->second.sizex1, bb.Min.y + it_anim->second.sizey1), ImVec2(bb.Max.x - it_anim->second.sizex1, bb.Max.y - it_anim->second.sizey1), ImColor(170,173,180,255), true, 10.f);
	ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.0f, 0.0f,  0.0f, it_nameanim->second));
    RenderTextClipped(bb.Min , bb.Max , ico, NULL, &label_size, style.ButtonTextAlign, &bb);
    ImGui::PopStyleColor();
...
bool ImGui::OptButton3(const char* ico, const ImVec2& size_arg, bool rotation)
{
    return OptButtonEx3(ico, size_arg, rotation, ImGuiButtonFlags_None);
}
...
struct subtab_element {
    float sizex, sizey, element_opacity, rect_opacity, text_opacity, text_opacity1,text_opacity2, text_opacity3;
};

bool ImGui::TabSubTab_Subtab(const char* name, bool boolean)
{
    ...
    //const ImVec2 label_size = ImVec2(285,60);//ImGui::CalcTextSize(name);//CalcTextSize(ico, NULL, true);
	const ImVec2 label_size = CalcTextSize(name, NULL, true);
	ImVec2 pos = window->DC.CursorPos;
	ImVec2 size = ImVec2( 40 , 40  );
    //const ImRect bb( pos, pos + size );
    const ImRect rect(pos, ImVec2(pos.x + label_size.x + 70, pos.y  + 50));
    ...
    it_anim->second.text_opacity1 = ImLerp(it_anim->second.text_opacity1, (boolean ? 100/255.f : 41/255.f), 0.03f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text_opacity2 = ImLerp(it_anim->second.text_opacity2, (boolean ? 103/255.f : 44/255.0f), 0.03f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text_opacity3 = ImLerp(it_anim->second.text_opacity3, (boolean ? 108/255.f : 49/255.0f), 0.03f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.text_opacity = ImLerp(it_anim->second.text_opacity, (boolean ? 1.0f : 1.0f), 0.07f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.sizex = ImLerp(it_anim->second.sizex, (boolean ?  0.0f : 4.0f), 0.15f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.sizey = ImLerp(it_anim->second.sizey, (boolean ?  0.0f : 1.0f), 0.15f * (1.0f - ImGui::GetIO().DeltaTime));
  	//==============
	window->DrawList->AddRectFilled(ImVec2(rect.Min.x ,rect.Min.y ), ImVec2(rect.Max.x ,rect.Max.y ), ImColor( 255/255.f,  255/255.f, 255/255.f, it_anim->second.sizey), 7.0f);
    window->DrawList->AddRectFilled(ImVec2(rect.Min.x + 18 ,rect.Min.y + 10 ), ImVec2(rect.Min.x + 22 ,rect.Max.y - 10 ), ImColor( 41/255.f,  44/255.f, 49/255.f, it_anim->second.sizey), 70.0f);
    window->DrawList->AddText( rect.Min + ImVec2(rect.GetSize().x / 2.f - CalcTextSize(name).x / 2.f - 5, rect.GetSize().y / 2.f - CalcTextSize(name).y / 2.f), ImColor(it_anim->second.text_opacity1, it_anim->second.text_opacity2, it_anim->second.text_opacity3, it_anim->second.text_opacity), name );
	return pressed;           																																																										
}
...
struct checkbox_animation {
    float animation,clr1,clr2,clr3, element_opacity, rect_opacity, text_opacity, text_opacity1,text_opacity2, text_opacity3;

};
bool ImGui::𝙆𝘼𝙐𝙎𝙃𝙄𝙆(const char* label, bool* v)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImVec2 posi = window->DC.CursorPos;
    const float jaat = GetWindowWidth() - 70;
    const ImRect HAXOR(posi, posi + ImVec2(jaat, 37));
    
    const float square_sz = 29;
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(w, square_sz + 8));
    const ImRect total_bb1(ImVec2(total_bb.Min.x + 0,total_bb.Min.y- 3), ImVec2(total_bb.Min.x + 1, total_bb.Min.y + 0) );
    const ImRect frame_bb(pos + ImVec2(w - square_sz - 23, 0), window->DC.CursorPos + ImVec2(w, square_sz - 1));
    ItemSize(HAXOR, style.FramePadding.y);
    if (!ItemAdd(total_bb, id))
    {
        IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags | ImGuiItemStatusFlags_Checkable | (*v ? ImGuiItemStatusFlags_Checked : 0));
        return false;
    ...
    bool hovered, held;
    bool pressed = ButtonBehavior(total_bb1,id, &hovered, &held);
    if (pressed)
    {
        *v = !(*v);
        MarkItemEdited(id);
    ...
    it_anim->second.text_opacity = ImLerp(it_anim->second.text_opacity, *v ? 1.f : 0.0f, 0.07f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.clr1 = ImLerp(it_anim->second.clr1, *v ? menu[0] : 41/255.0f, 0.07f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.clr2 = ImLerp(it_anim->second.clr2, *v ? menu[1] : 44/255.0f, 0.07f * (1.0f - ImGui::GetIO().DeltaTime));
    it_anim->second.clr3 = ImLerp(it_anim->second.clr3, *v ? menu[2] : 49/255.0f, 0.07f * (1.0f - ImGui::GetIO().DeltaTime));
    
    
    
    RenderNavHighlight(total_bb, id);
    
    window->DrawList->AddRect( ImVec2(total_bb.Min.x + 30,total_bb.Min.y- 3), ImVec2(total_bb.Min.x + 60, total_bb.Min.y + 27) , ImColor( 41, 44, 49, 255), 7.f, ImDrawFlags_RoundCornersAll, 3.0f);
    window->DrawList->AddRectFilled( ImVec2(total_bb.Min.x + 31,total_bb.Min.y-2), ImVec2(total_bb.Min.x + 59, total_bb.Min.y + 26) , ImColor( 41/255.0f, 44/255.0f, 49/255.0f, it_anim->second.animation), 7.0f, ImDrawFlags_RoundCornersAll);
    RenderCheckMark( window->DrawList, { total_bb.Min.x + 35.7, total_bb.Min.y + 1.5 }, ImColor( 1.0f, 1.0f, 1.0f, it_anim->second.animation), 20.f );
    
    
    /*
    window->DrawList->AddRect(ImVec2(frame_bb.Min.x, frame_bb.Min.y), ImVec2(frame_bb.Max.x, frame_bb.Max.y), ImColor(it_anim->second.clr1, it_anim->second.clr2, it_anim->second.clr3, 1.0f), 50.f,ImDrawFlags_RoundCornersBottomLeft | ImDrawFlags_RoundCornersTopLeft | ImDrawFlags_RoundCornersBottomRight | ImDrawFlags_RoundCornersTopRight,4.0f);
    //window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 4, frame_bb.Min.y+4), ImVec2(frame_bb.Max.x-4, frame_bb.Max.y-4), ImColor(0/255.0f, 0/255.0f, 0/255.0f, 1.0f), 50.f,ImDrawFlags_RoundCornersBottomLeft | ImDrawFlags_RoundCornersTopLeft | ImDrawFlags_RoundCornersBottomRight | ImDrawFlags_RoundCornersTopRight); 
    //window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x - 1, frame_bb.Min.y - 1), ImVec2(frame_bb.Max.x + 1, frame_bb.Max.y + 1), ImColor(menu[0]/2, menu[1]/2, menu[2]/2, it_anim->second.text_opacity), 50.f,ImDrawFlags_RoundCornersBottomLeft | ImDrawFlags_RoundCornersTopLeft | ImDrawFlags_RoundCornersBottomRight | ImDrawFlags_RoundCornersTopRight);
    window->DrawList->AddCircleFilled(ImVec2(frame_bb.Min.x + 15.0 + 22.04 * it_anim->second.animation, frame_bb.Min.y + 13.5), 7.5f,ImColor(41, 44, 49, 255), 70);
   */
           
    {
        ...
        PushStyleColor(ImGuiCol_Text, ImColor(100, 103, 108, int(255 * fill)).operator ImVec4());
        RenderText(ImVec2(total_bb.Min.x + 67, total_bb.Max.y - label_size.y - 13), label);

        PushStyleColor(ImGuiCol_Text, ImColor(41, 44, 49, int(100 * it_hovered->second)).operator ImVec4());
        RenderText(ImVec2(total_bb.Min.x + 67, total_bb.Max.y - label_size.y - 13), label);

        PushStyleColor(ImGuiCol_Text, ImColor(41, 44, 49, int(255 * it_active->second)).operator ImVec4());
        RenderText(ImVec2(total_bb.Min.x + 67, total_bb.Max.y - label_size.y - 13), label);
    }
...
bool ImGui::𝙆𝘼𝙐𝙎𝙃𝙄𝙆_𝙉𝙀𝙒(const char* label, bool* v)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    window->DrawList->AddRect( ImVec2(total_bb.Min.x + 30,total_bb.Min.y- 3), ImVec2(total_bb.Min.x + 60, total_bb.Min.y + 27) , ImColor( 0, 0, 0, 255), 7.f, ImDrawFlags_RoundCornersAll, 3.0f);
    window->DrawList->AddRectFilled( ImVec2(total_bb.Min.x + 31,total_bb.Min.y-2), ImVec2(total_bb.Min.x + 59, total_bb.Min.y + 26) , ImColor( 0/0.0f, 255/255.0f, 0/0.0f, it_anim->second.animation), 7.0f, ImDrawFlags_RoundCornersAll);
    RenderCheckMark( window->DrawList, { total_bb.Min.x + 35.7, total_bb.Min.y + 1.5 }, ImColor( 0.0f, 0.0f, 0.0f, it_anim->second.animation), 20.f );
...
template<typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb, float* percent )
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
...
bool ImGui::SliderBehavior(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, void* p_v, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb, float* percent )
{
    // Read imgui.cpp "API BREAKING CHANGES" section for 1.78 if you hit this assert.
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
	...
	const ImVec2 posi = window->DC.CursorPos;
	const float jaat = GetWindowWidth() - 30;
	const ImRect HAXOR(posi, posi + ImVec2(jaat, 37));
    const ImVec2 label_size = CalcTextSize( label, NULL, true );
    const ImRect frame_bb( window->DC.CursorPos, window->DC.CursorPos + ImVec2( w, 27 ) );
    const ImRect slider_bb(posi + ImVec2(30,0), posi + ImVec2(jaat, 37));
    ...
    AnimationHandle->second.slider_bg = ImLerp( AnimationHandle->second.slider_bg, hovered || g.ActiveId == id ? ImColor( 42, 44, 48, int( 255 * g.Style.Alpha ) ).Value : ImColor( 24, 24, 27, int( 255 * g.Style.Alpha ) ).Value, g.IO.DeltaTime * 8.f * g.IO.AnimationSpeed );
    AnimationHandle->second.Text = ImLerp( AnimationHandle->second.Text, hovered || g.ActiveId == id ? Colors::TextActive : Colors::Text, g.IO.DeltaTime * 8.f * g.IO.AnimationSpeed );
    AnimationHandle->second.hovered = ImLerp( AnimationHandle->second.hovered, hovered || g.ActiveId == id ? 1.f : 0.f, g.IO.DeltaTime * 8.f * g.IO.AnimationSpeed );
    AnimationHandle->second.slider_grab = ImLerp( Colors::AccentDisabled, Colors::Accent, AnimationHandle->second.hovered );
    //AnimationHandle->second.mark_bg = ImLerp( AnimationHandle->second.mark_bg, g.Style.Alpha >= 0.5f && can_add && !window->SkipItems && *v ? ImColor( ( 130 / 3.44f ) / 255.f, ( 132 / 3.44f ) / 255.f, ( 170 / 3.44f ) / 255.f,  g.Style.Alpha ).Value : g.Style.Alpha >= 0.5f && can_add && !window->SkipItems && ( hovered || hovered_mark ) ? ImColor( 42, 44, 48, int( 255 * g.Style.Alpha ) ).Value : ImColor( 24, 24, 27, int( 255 * g.Style.Alpha ) ).Value, g.IO.DeltaTime * 8.f );

    // Draw frame
    RenderNavHighlight( slider_bb, id );
    RenderFrame( ImVec2( slider_bb.Min.x , slider_bb.GetCenter( ).y + 4.0f ), ImVec2( slider_bb.Max.x, slider_bb.GetCenter( ).y + 14.0f ), ImColor( 150,150,150,255 ), false, 100.f );

    // Slider behavior
    ImRect grab_bb;
    float percent = 0;
    bool value_changed = temp_input_is_active || temp_input_is_active ? false : SliderBehavior( slider_bb, id, data_type, p_data, p_min, p_max, format, ImGuiSliderFlags_None, &grab_bb, &percent );
    if ( value_changed && !temp_input_is_active && !temp_input_is_active )
        MarkItemEdited( id );
    if ( !( temp_input_is_active || temp_input_is_active ) )
        AnimationHandle->second.position = ImLerp( AnimationHandle->second.position, g.Style.Alpha < 0.5f || !can_add ? 0.f : ( float ) percent, g.IO.DeltaTime * 12.f * g.IO.AnimationSpeed );

    // Render grab 
    {
        window->DrawList->AddRectFilled( ImVec2( slider_bb.Min.x , slider_bb.GetCenter( ).y + 4.0f  ), ImVec2( slider_bb.Min.x  + ( slider_bb.GetSize( ).x - 0.f ) * AnimationHandle->second.position / 100.f, slider_bb.GetCenter( ).y + 14.0f ),ImColor( 41, 44, 49,255 ), 100.f );
        window->DrawList->AddCircleFilled( ImVec2( slider_bb.Min.x  + 8.0f + ( slider_bb.GetSize( ).x - 18.f ) * AnimationHandle->second.position / 100.f, slider_bb.GetCenter( ).y + 9.0f ), 12.5f, ImColor( 255, 255, 255, 255 ), 24 );
    }
    ...
    if ( label_size.x > 0.0f )
        window->DrawList->AddText( ImVec2( HAXOR.Min.x + 30.f, frame_bb.GetCenter( ).y - CalcTextSize( label ).y / 2.f - 10.0f), AnimationHandle->second.Text, label );
   ...
   // PushFont( g.IO.Fonts->Fonts[ 1 ] );
    window->DrawList->AddText( text.GetCenter( ) - CalcTextSize( value_buf ) / 2.f - ImVec2(25,10), ImColor( Colors::TextActive ), value_buf, value_buf_end );
   // PopFont( );
...
bool ImGui::VSliderScalar(const char* label, const ImVec2& size, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    float percent = 0;
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(frame_bb, id, data_type, p_data, p_min, p_max, format, flags | ImGuiSliderFlags_Vertical, &grab_bb, &percent);
    if (value_changed)
        MarkItemEdited(id);
